...
const int TIME_LIM = 3600; /*所定义的程序允许时间上限是60分钟，也就是一个小时。*/

int N, M, K, wG; /* N 和 M，分别表示图的顶点数和边数。K表示max s-bundle中的S值，wG表示s-bundle的下界。*/
pair<int, int> *E; /*边的集合，存放的是顶点对。*/

// gets() is deprecated
#define gets(str) (fgets(str, 200, stdin)) 
/*这个宏的目的是为了替换掉标准库中的gets()函数，因为gets()函数由于不检查缓冲区的大小而容易导致缓冲区溢出，从而引发安全问题。通过定义这个宏，可以使代码更安全，同时还能保持使用gets()函数的简洁性。*/

void read_clq() /*clq是一种二进制的数据库文件。*/
{
    char str[200], stmp[10];  /*我没有想到这些代码里面居然还包括了去除重复边！*/ 
    while (true)
    {
        gets(str);
        if (*str == 'p')
...
{
    string str;
    getline(cin, str); /*getline函数会从输入流is中读取字符，直到遇到指定的分隔符delim（默认为换行符），并将读取的字符存储在字符串对象str中。它还会从输入流中丢弃分隔符。*/
    stringstream fs; /*stringstream允许我们以类似于处理文件流的方式处理字符串，提供了方便的字符串输入/输出功能。stringstream类位于std命名空间中，并且它是从istream和ostream类派生而来的*/
    fs << str; /*因此它继承了这两个类的输入和输出功能。这使得我们可以使用<<和>>运算符来进行字符串的输入和输出操作。*/
    fs >> N >> M;  // 从 fs 中提取两个整数 N 和 M，分别表示图的顶点数和边数。注意，M 这里被提取，但后面的代码会进行修正，因为它可能不是准确的边数。
    vector<pair<int, int>> vedges;
    static set<int> st; /*一个set集合，用来存放不相同的元素，主要是用来避免存放相同的顶点。*/
    for (int i = 0; i < N; ++i) /*遍历所有的顶点（顶点数量是从文件中读取的）。*/
    {
        getline(cin, str);
        stringstream ss;
        ss << str;
...
/*twoPow是一个整数数组。1 << 16 是一个位移操作，意味着数字 1 向左移动 16 位。在大多数现代架构中，int 是32位，这意味着 1 << 16 将会是 65536（即 2 的 16 次方）。
...
具体来说，这段代码的作用是初始化一个数组 twoPow，该数组的索引表示 2 的幂，而数组中的元素表示对应的指数。换句话说，它建立了一个映射关系，用于将 2 的幂映射到对应的指数值。
例如twoPow[1]将被设置为 0，twoPow[2]将被设置为 1，twoPow[4] 将被设置为 2，以此类推。这种映射通常用于一些算法中，特别是在处理二进制位操作时。例如，如果你想知道某个数是 2 的多少次方，
你可以使用这个数组来查找。例如，如果twoPow[16]的值为 4，那么你可以知道 2^4 = 16。这对于一些位操作和计算中非常有用。 */
...
struct BIT_SET /*位图运算，可以快速的判断出一个顶点的邻居和非邻居。*/
{
    ...
    int lb(unsigned x) /*lb 函数根据 x 的值在twoPow中查找相应的结果，并根据x的哪一部分（高16位或低16位）包含有效位来调整这个结果。*/
    {
        if (x & msk)   /*它检查 x 的低16位是否有任何位为1。*/
            return twoPow[x & msk]; /*如果 x 的低16位中有设置的位，函数返回twoPow数组中相应索引处的值。*/
    ...
    int lowbit() /*此函数的目的是查找并返回集合中从右往左数第一个非零位的位置。它使用了位运算来高效地完成这项任务。*/
    {
        for (int i = 0; i <= n; ++i)
        {
            ...
            if (x) /*如果 x 不为零，表示在当前的 buf[i] 中找到了非零位。*/
                return lb(x) + (i << 5); 
                /*lb(x) 返回的是 x 中的最低位的非零位在其32位字中的位置（从0开始计数）。由于我们只关心 x 的最低位的非零位，所以 lb(x) 的值范围是0到31。
                (i << 5) 这部分是将 i（当前元素的索引）左移5位，相当于乘以32。这是因为我们假设每个 buf[i] 的长度是32位。所以，这个操作实际上是将索引转换为位位置。
                将上述两个结果相加，即 lb(x) + (i << 5)，就得到了在整个集合中从右往左数第一个非零位的绝对位置。*/
        }
        return -1; /*如果没有找到一个非零位，那么就返回-1.*/
    }
    /*关于为什么可以像上面这样做的一个解释是：当我们处理二进制数据时，我们经常需要将某个索引或位置转换为对应的位位置。这是因为在计算机内部，数据是以二进制形式存储的，每个数据单元
    （比如一个字节）包含多个位（bit）。每个buf[i]元素是一个固定长度的二进制数，比如32位（这是一个常见的长度，但也可以是其他长度）。我们想要找到的是从整个数据集（所有buf元素的组合）
    的最右边开始数的第一个非零位。为了实现这一点，我们需要知道每个buf[i]元素在整个数据集中所占的位范围。如果一个元素是32位长，那么第一个元素buf[0]的位范围是0-31，第二个元素buf[1]
    的位范围是32-63，以此类推。因此，当我们说“将索引转换为位位置”时，实际上是指将数组索引转换为其对应的位范围起始点。这个转换通常是通过将索引乘以每个元素的位数（在本例中是32位，或
    者说左移5位，等同于乘以32）来实现的。这样，我们就能快速定位到每个buf[i]元素在整体二进制表示中的起始位置。*/
...
public:
    ...
    /*这段代码实现了网络流中的SAP（Shortest Augmenting Path）算法，用于计算从起点start到终点end的最大流。代码中使用了很多网络流算法的标准技术，例如前向边和反向边、层次图（level graph）、
    以及GAP优化。该算法会返回一个int类型res，表示最大流的结果，也就是最小割的数值。注意了！！在求解具有连通度属性的max s-bundle问题的时候，最重要的一个定理依据就是“最大流最小割”定理！
    也就是说s-bundle是想找出来最小割的大小，然而我们要是直接求解最小割的话会非常的不方便，所以说我们就求解了最大流，因为最大流问题有非常成熟的方法来进行求解。该算法采用的是Ford-Fulkerson算法
    和SAP（Shortest Augmenting Path，最短增广路径）算法的结合。关于Ford-Fulkerson算法的详细说明：
    第一步：构建一个残差图，根据图中边的容量来初始化所有的残差。
    第二步：如果该图中存在着增广路径，则：
    1）在残差图中寻找一条增广路径；
    2）寻找该条增广路径上的瓶颈容量x；
    3）更新剩余的残差；即（residual = residual - x);
    4）增加一条反向路径，在该路径上，所有的边的权重都是x。
    不过需要特别注意的一点是！！！本论文中的SAP算法的实现不是基于所谓的Ford-Fulkerson算法和Emonds-card算法，而是更加高效的dinic's算法，因为有层级的概念！！
    所谓的Dinic's算法的具体步骤是：
    ...
    {
        for (int i = 0; i < tol; ++i)  /* 恢复每条边的初始容量，为新的流计算做准备*/
        ...
        /*调用了BFS函数后，我们可以得到两个非常关键的数组：（1）dep，用于存储从源点到每个点的最短路径长度；（2）gap，用于存储到达每个点的增广路径的数量。*/
        int top = 0; /*记录当前遍历到的边*/
        memcpy(cur, head, n * sizeof(int)); /*复制head数组到cur数组。*/
        int u = start; /*初始化u为源节点*/
        int i;
        while (dep[start] < n) /*当源节点的最短路径长度小于n时，继续循环*/
        {                      
            ...
            for (i = cur[u]; i != -1; i = edge[i].next)  /*查找当前节点u指向的下一个合适的边*/
                if (edge[i].cap != 0 && dep[u] == dep[edge[i].to] + 1) /*如果当前边有容量并且指向的节点与当前节点在同一层，则退出循环。*/
                    break;
            if (i != -1) /*如果找到了合适的边（即前面的for循环中没有出现异常情况break掉），前进到该边的终点*/
            {
                cur[u] = i; /*更新当前节点记录的边*/
...
public:
    ...
    该函数有两种不同的模式，取决于参数 cals 的值。（1）如果 cals 为真，函数用于检查子图 svex是否是一个符合要求的最小割。（2）如果cals为假，函数用于检查整个图是否是最小割。
    传入的两个参数中，第一个参数是是否仅仅计算当前解集svex的最小割；第二个参数是当前的下界bound。经过漫长的试错，我终于搞懂了这个bound的具体作用！由于我们求解的是max s-bundle问题，
    而s-bundle指的是最少移除|V|-s个顶点后图不连通，而这个bound就是指的|V|-s，所以和s-bundle中的s相关的参数就体现在solve函数的bound参数中。*/
    bool solve(bool cals, int bound) 
    {
        int nn = 0; /*定义并初始化一个局部变量 nn，用于跟踪数组 nV 和 oID 的填充情况。*/
        if (cals)   /*如果 cals 为真，则计算当前解集svex的最小割*/
        {
            for (auto u : svex) /*遍历当前解集svex中的每一个顶点。*/
            ...
            for (auto u : svex)   /*再次遍历 svex 中的每个顶点，需要注意的是此时的nn已经变成了svex的大小了！*/
                mf.addedge(nV[u], nV[u] + nn, 1); /*这里是添加正向边，添加从顶点到其对应超级汇点的边，边的容量设置为 1。*/
            for (auto u : svex) /*接下来的双重循环是用来寻找解集svex中两两顶点之间是否相邻。*/
            {
                for (auto v : svex)
                {
                    if (!adjN[u].test(v)) /*如果顶点u和顶点v不相邻，则跳过这一次循环。所以说只给相邻的顶点添加反向边。*/
                        continue;
                    mf.addedge(nV[u] + nn, nV[v], INF); /*如果两个顶点相邻，则为所有相邻顶点对添加无限大容量的反向边。*/
                }
            }
            int u = nV[svex.back()];     /*选取 svex 向量中最后一个顶点的新编号 u。back方法返回一个指向std::vector中最后一个元素的迭代器*/
            for (int i = 0; i < nn; ++i) /*对于 nn 范围内（也就是当前的解集svex向量的大小）的每个顶点 i，运行SAP算法计算从超级源点 u + nn 到 i 的最大流。*/
            {
                if (i == u || adjN[oID[u]].test(oID[i])) /*如果该顶点是顶点 u自身，或者u与i邻接*/
                    continue; /*则跳过这一次循环。！！！所以说在运行SAP算法的时候主要看的是和顶点u不相邻的顶点！！！这里和前面添加反向边时的规则相反。*/
                if (mf.SAP(u + nn, i) < bound) /*如果结果小于 bound，则返回 false，表示无法找到大小为bound的一个最小割。注意，这里调用了SAP算法，SAP会返回一个最大流的数值，表示最大流量，
                这个数值等于最小割，也就是我们问题中所要求解的地方。u+nn表示反向流量。*/
                    return false;
            }
            return true; /*如果所有测试都通过，返回 true，表示图符合大小为bound的最小割。*/
        }
        else /*else目的是检查图的全局最小割，而不是 svex 子图的最小割。通过对所有未删除顶点进行网络流计算，如果在任何情况下最小割小于 bound，则图的最小割认为是小于bound，
        则表示不符合我们所要求解的max s-bundle的要求。*/
        {
            for (int i = 0; i < n; ++i)
                if (!del[i])   /*如果顶点 i 没有被删除（即 del[i] 为假）*/                       
                    nV[i] = nn, oID[nn++] = i;        /*则将 i 存储在 nV 中，并且在 oID 中以新的编号 nn 存储原始编号 i，nn 递增。*/
            mf.init(nn + nn);   /*初始化网络流对象 mf，节点数量为 nn + nn（这里的nn表示当前图中所有有效的顶点大小），这包括了为每个顶点添加的超级源点和超级汇点。*/
            ...
            for (int i = 0; i < n; ++i)  /*对于每个未删除的顶点 i，遍历其邻接表中的所有边*/
            {
                if (del[i]) /*这个条件语句检查顶点 i 是否已被删除，如果已删除，则跳过当前顶点的处理。*/
                    continue;
                for (int e = head[i]; ~e; e = nxt[e]) /*如果顶点i没有被删除，则遍历顶点 i 的邻接表中的所有边。*/
                {
            ...
            /*上面的操作都是在构建一个最大流的结构，现在已经完成了对正反两个方向的边的初始化和添加。接下来将使用SAP算法来计算图中的最小割是否达到了bound*/
            for (int i = 0; i < nn; ++i)  /*外层循环，用于遍历顶点编号 i，其中 nn 是未删除顶点的数量。*/
                for (int j = i + 1; j < nn; ++j) /*内层循环，用于遍历顶点编号 j，确保 i 和 j 不是同一个顶点，避免重复检查。*/
                    if (!adjN[oID[i]].test(oID[j]) && mf.SAP(nn + i, j) < bound) /*如果两个顶点不相邻且最小割小于bound（（所以充分说明了，只有对于不相邻的两个顶点才会调用SAP算法来计算最小割）*/
                        return false; /*将计算得到的最大流值与指定的 bound 进行比较，看是否小于 bound。如果小于 bound，表示从超级源点到达顶点 j 的最大流小于 bound，即移除顶点 oID[i] 后，图的顶点连通性小于 bound。*/
            return true; 
        }
    }
} vc;
...
{
    nxt = new int[M + M];      /*首先进行了内存分配,M 是边的数量，而 N 是顶点的数量，nxt 和 to 用于存储邻接链表的信息，*/
    to = new int[M + M];
    ...
    nV = new int[N];            /*nV(new vertices)用于存储根据顶点度数删减后的顶点集合。*/
    memset(degree, 0, N * sizeof(int)); /*degree 数组被初始化为零（表示所有顶点的初始度数为零）*/
    memset(head, -1, N * sizeof(int));  /*head 数组被初始化为 -1（表示链表的开始）*/
    etot = 0;                           /*etot 被设置为 0，它表示边的总数，全称是edge-total*/
    for (int i = 0; i < M; ++i) /*这个循环处理图中的每一条边，增加相关顶点的度数，并通过addedge函数添加边。 这里是在构建一个无向图，因为它同时添加了u到v和v到u的边。*/
    {       
    ...
    while (fr ^ re) /*循环删除当前集合中顶点度数不符合kplex要求的顶点用异或运算来检查队列是否为空，开始进行宽度优先搜索。异或操作的技巧是不同为1，相同为0。*/
    {                      
    ...
    for (int i = 0; i < N; ++i) /*现在开始把没有被删除的顶点放在新数组里面*/
        if (!outcore[i]) 
    ...
    int m = 0; /*初始化边的计数器 m。*/
    for (int i = 0; i < M; ++i) /*循环遍历所有的边（由数组 E 存储）*/
    {  
        int u = E[i].first, v = E[i].second;
        if (outcore[u] || outcore[v]) /*对于每一条边 (u, v)，如果它的任一端点 u 或 v 被标记为 outcore（即它们中的至少一个不在核心子图中），则跳过这条边。*/
            continue; 
    ...
    ordID = new int[n];
    dis = new int[n];
    mf.reserve(n, m); /*初始化最大流对象。*/
    vc.reserve(n); /*初始化顶点连通度对象。*/
    return true;
}

void getOrd() /*getOrd 函数的主要作用是获得退化排序（degeneracy ordering）。*/
{  
    memset(degree, 0, n * sizeof(int));  /*初始化度数数组degree为0，头指针数组head为-1，边的总数etot为0。这些操作准备数据结构，以存储图中每个顶点的邻接信息。*/
    memset(head, -1, n * sizeof(int));
    etot = 0; // etot 用于记录边的总数。
    for (int i = 0; i < M; ++i)  /*遍历所有边，更新顶点的度数，并通过addedge函数将边添加到邻接表中。*/
    { 
        int u = E[i].first, v = E[i].second; /*更新顶点的度数数组，因为前面已经删除过一些顶点了，所以需要重新更新。*/
        ++degree[u];
        ++degree[v];
        addedge(head + u, v); // 调用 addedge 函数，将 v 添加到 u 的邻接表中，同时将 u 添加到 v 的邻接表中。
        addedge(head + v, u); // 在addedge函数里面，etot的数量进行了自增。
    }
    static priority_queue<pair<int, int>> Q; // Q用于存储顶点和它们的度数，它是优先队列（最大堆），其中元素为 pair<int, int> 类型，表示顶点的度数和顶点的编号。
    /*这是一个通用的优先队列，可以容纳任何类型的元素。元素按升序排列，也就是说，默认队列的顶部总是包含最大的元素。关于优先队列的常用成员函数：empty() 如果优先队列为空，则返回真；
     pop() 删除第一个元素 ；push() 加入一个元素 ；size() 返回优先队列中拥有的元素的个数； top() 返回优先队列中有最高优先级的元素。 */
    memset(outcore, 0, n * sizeof(bool)); /*这里的outcore数组就是一种变了形式的visited数组，只不过这里强调了它的度数概念。*/    
    ...
    {
        int u = Q.top().second; /*取出队列顶部的顶点u（度数最小的顶点）并将其从队列中移除。second表示顶点的编号。*/
        Q.pop();     
    ...
    for (int i = 0; i < n; ++i)  /*初始化一个数组ordID，用于存储顶点的新顺序。*/
        ordID[i] = i;   
    /*然后根据顶点的up_bound（其实就是core值）来对顶点进行升序排序（其实就是对顶点进行退化排序）。ordID和ordID+n的意思就是数组的第一个元素到数组的最后一个元素，简单来说就是针对的是整个数组。*/
    ...
    inD.flip();  /*使用flip()函数对位集合进行翻转，将所有位的值从0变为1，表示刚开始所有的顶点都位于候选集D中。*/
    inG.init(n); /*同样初始化了另一个位集合inG*/
    inG.flip(); /*表示所有顶点刚开始都是在图中的状态。*/
...
void exit_program() 
{ /*退出程序的时候打印信息：（1）文件的名称filename；（2）s-bundle的s的值K；（3）深度优先搜索经历过的顶点数量dfs_node；（4）我们传入的s-bundle的下界LB；（5）算法运行所消耗的时间。*/
...
inline void addtoS(int u) /*用于将一个顶点u添加到已选集S中。*/
{
...
inline bool canadd(int u) /*检查是否可以将节点 u 添加到当前的 svex 集合中，而不违反算法的约束条件。判断包括两个部分，第一是判断是不是一个kplex，第二则是判断是否连通且最大流是否满足条件。*/
{
    int tot = 0; /*用于计算节点 u 与当前 svex 集合中的节点不相邻的数量。*/
    for (auto v : svex)
        if (!adjN[u].test(v)) /*如果顶点u和顶点v不相邻，则执行以下操作：*/
        {
            if (++tot >= K) /*如果不相邻的节点数量大于等于 K，表示添加节点 u 将导致超过 K 个不相邻的节点存在于 svex 集合中，这将违反算法的约束条件，因此返回 false。这里是根据kplex的性质进行的判断。*/
                return false;
            if (notadj[v] >= K - 1) /*如果节点 v 的不相邻节点数量大于等于 K-1，表示添加节点 u 可能导致节点 v 成为一个不相邻的节点，这也将违反算法的约束条件，因此返回 false。*/
                return false; /*第二个if的目的是尽可能提前的把不符合kplex要求的情况筛选出去。*/
        }
    svex.push_back(u); /*如果前面没有问题，那么就可以把这个顶点暂时加到我们的解集svex中。*/
    bool ret = vc.solve(true, svex.size() - K); /*暂时加入了顶点u之后我们还要进一步的判断这个图是不是一个符合bound要求的最小割，调用solve方法，设置标记为true，表示只检查svex的最小割。*/
...
void bfs(int s) /*广度优先搜索算法用于计算从节点s出发到其他节点的最短距离，并将结果存储在 dis 数组中。便于后面进行和距离相关的一些剪枝操作，这些都在论文中有提及。*/
{
    memset(dis, -1, n * sizeof(int)); /* 初始化 dis 数组，并将所有节点的距离初始化为 -1，表示尚未访问。*/
    ...
    while (fr ^ re)                   /*进行广度优先搜索。*/
    {
        ...
        {
            int v = to[e];
            if (del[v] || dis[v] != -1) /*如果顶点v已经被删除或者是已经被处理过一次了，则跳过当前循环：*/
                continue;
            dis[v] = dis[u] + 1; /*否则就设置节点 v 的距离为节点 u 的距离加 1，函数的思想就是通过bfs来对计算距离我们传入的顶点s的其他顶点的距离，同时也判断了整个图的连通性了。*/
            que[re++] = v;  /* 计算完1跳邻居后，将节点放入队列以进行后续距离计算的处理。*/
        }
    }
}

void check_timeout() /*该函数用来检查程序运行时间是否超过所规定的值。*/
{
    if ((double)(clock() - startTime) / CLOCKS_PER_SEC > TIME_LIM)
        exit_program();
}

int get_color() /*这个函数对应于作者在论文中提到的基于图着色的解的上界，在3.3节。具体的求解方法描述在原文中非常详细。这个无参的get_color函数就是计算出来一个上界值，然后进行返回。*/
{
    BIT_SET Q, P;
    ...
    while (!P.empty())
    {
        int cnt = 0;
        P.cp(Q);
        do /*现在这里完全对应着论文中的lemma2，计算出所有的独立集里面能够提供的上界。*/
        {
            int u = Q.lowbit();
            ++cnt;
...
{
    BIT_SET Q, P; /*初始化 BIT_SET 对象并拷贝状态。根据作者在论文里面所说的，采用位操作来计算图着色上界会非常的快。*/
    inD.cp(P); /*把inD拷贝到刚刚创建的集合P中。注意，inD表示在候选集合中，这就相当于作者在论文里面所说到的C集合。*/
    int tot = 0; /*total的意思就是指当前的图的集合能够获得的基于染色的最大上界。*/
    while (!P.empty())  /*循环处理直至P为空，其中P是inD的副本，表示删除顶点集合。P.empty() 检查 P 是否没有任何设置的位。*/
    { /*在第一个大循环中，每次都是找出来一个独立集。*/
        int cnt = 0;
        P.cp(Q); /*在每一次循环中，把位集合P中的内容复制到集合Q中。*/ 
        do/*在 Q 中进行操作并更新 lst*/
        {
            int u = Q.lowbit(); /*获取Q中从右往左数的第一个1的位置，并赋值给 u。假设此时的u为u1，那么当进行第二次while循环时候得到的u2则是u1的非邻居顶点了。*/
            ++cnt; /*计数自增。*/
            if (cnt > K) /*如果超过K，则将染色计数cnt设置为K，这是基于图着色上界的一种常用方法。*/
                cnt = K;
            ...
            执行Q &= invN[u]操作的意思是找出集合Q里面的顶点和顶点u不相邻的顶点且这些顶点也是存在Q里面的。简单来说，执行完这句后，Q里面剩下的顶点都是和顶点u不相邻的顶点。*/
        } while (!Q.empty());
        tot += cnt; /*需要注意的是，无论上面的独立集的大小是多少，由于执行了一个界限判断操作，传出来的cnt的大小最多就是K，也就是我们要求的最大s-bundle中的s的值。
        解释一下，之所以每一次都要计算一个tot，并且把它和顶点的编号一同压入lst中的目的是，tot表示当前最大的图着色上界！因为是从前往后放进去的，而后面的函数中是从后往前取的，
        所以每个后面的顶点的tot都表示当前lst集合中剩余的所有的顶点能够提供的最大可能解的大小就是tot，借此来判断最优解的上界，从而方便下界剪枝。*/
    }
}

/*这个dfs是在当前的图是一个合格的kplex的情况下调用的，属于dfs里面的一种特殊情况，因为只有当当前的图是一个kplex，那么它才有可能是一个s-bundle，反之则不然。*/
void dfs_kbundle(int curS, bool check) /*第一个参数表示当前的顶点集合大小；第二个参数表示是要检查当前解集svex是一个最小割，还是要检查整个图是一个最小割。*/
{
    check_timeout();
    ++dfs_node; /*递归次数加1。*/
    ...
    if (curS <= LB)
        return;
    if (check && vc.solve(false, curS - K)) /*如果 check 为真且solve函数返回真（意味着当前的curS构成的图是一个符合curS-K大小的最小割），就说明当前的图是一个合格的s-bundle！*/
    { /*作者的思路是不断的将当前的解空间的大小减少1，以这样的方式来判断是否存在最小割为bound的一个图，所以说solve只能求解是不是传进来的这个大小的割，不能确定是不是最大的。*/
        LB = curS; /*现在的LB就是真的和s-bundle相关的下界了*/
        return;
    }
    /*获取颜色列表并进行检查*/
    vector<pair<int, int>> lst; /*定义了一个顶点对集合lst，这个集合里面存储了候选集D中每个顶点的最大染色上界数。*/
    get_color(lst); /*关于lst集合的赋值操作在get_color函数中进行，运行完这个函数后，get_color函数中装满了每个顶点的染色上界。*/
    int m = lst.size(); /*获得lst集合的大小，根据get_color函数里面的实现，每次放进lst里面的都是一个顶点，所以这里的m就表示候选集中的顶点剩余数量。*/
    if (m == 0) /*如果 lst 为空，就意味着整个候选集为空，则算法结束。*/
        return;
    check = false; /*当check为假的时候，意味着再次调用dfs_kbundle的时候，solve函数将会检查整个图的最小割是不是bound的大小，而不仅仅是检查当前的解集svex的最小割。*/
    for (int i = m - 1; i >= 0; --i) /*进行反向遍历。注意了！这里是进行的反向遍历！目的就是试探出最多能够加入到解集S中的界限值！先从最后一个元素遍历，这时候的lst[i]是最大的。*/
    {
        if ((int)svex.size() + lst[i].second <= LB) /*lst.second存放的是和顶点数量相关的。这个if语句表达的意思是如果lst[i]（即候选集中的顶点i的最大上界）无法*/
        { /*取得一个比当前下界更优的解的时候，就意味着这时候的i就是一个临界值，就把剩余的顶点都加入到候选集D中。*/
            for (int j = i + 1; j < m; ++j)
                addtoD(lst[j].first); /*重新把这些顶点放回到候选集合中去，D表示候选集。因为在循环后面调用了delfrD将这些顶点从候选集中删除掉了。*/
            return; /*提前终止函数。*/
        }
        /*如果没有提前终止函数，则说明这个时候的上界是符合最优解要求的，把这个顶点加入到解集中是有可能超过当前的历史最优解的。*/
        if (canadd(lst[i].first)) /*如果经判断，该顶点可以被添加到解集svex中不违反kplex的性质并且还是符合bound要求的最小割的话，则进行真正的添加进svex中。*/
        {
            addtoS(lst[i].first); /*addtoS表示把一个顶点添加到解集中去，S是solution的首字母缩写。*/
            dfs_kbundle(curS, check); /*进行递归操作。我终于明白了，每次递归执行完还需要将环境等进行恢复。*/
    ...
    for (int i = 0; i < m; ++i)/*循环结束后，重新把染色集合lst中的所有元素放回到候选集合D中去*/
        addtoD(lst[i].first);
}
...
void dfs(int curS) /*传入的参数curS是当要搜索的顶点集合的大小。*/
{
    ...
    if (curS <= LB) /*递归终止条件！！如果 curS（当前解）小于等于 LB，则函数返回，停止进一步搜索。*/
        return; 
    ...
    {  
        ...
        /*需要说明的是，在递归函数里面，并不是说一个return就会终止整个函数！而是return会不断的把值往回抛！但是直到满足递归终止条件的时候才会结束算法！！！也就是说无论上面的dfs返回的是
        什么，后面的addtoD都会被执行。*/
        return;
    }
    /*经过前面的代码后，现在的minID的顶点度数既不符合kplex的要求，但是也不比LB+1-K小。那么就执行另外一种情况：寻找未被删除的度数最大的顶点*/
    int maxID = -1; /*初始化一个变量maxID，用于存储度数最大的未删除顶点的编号。*/
    for (int i = 0; i < n; ++i)
        if (!del[i]) /*如果该顶点没有被删除。需要说明的是，在整个代码中，一个顶点只有在delfrD中才会被删除，因为只有这个函数里面有设置del为true的命令。*/
        {
    ...
    if (!ins[maxID] && notadj[maxID] >= K) /*如果最大度数的顶点maxID没有被插入到解集S中并且与maxID不相邻的顶点数量大于等于 K（说明不符合基本的kplex的要求）*/
    {
        ...
        return; /*这一段代码的目的是考虑了一种情况，即顶点maxID可以被删除以获得更优解的可能性*/
    }
    /*第二种情况：当前度数最大的顶点已经在解集S中，并且它的度数已经快要不满足kplex的要求了。这里对应着论文中原文的3.4部分的Reduction，属于饱和顶点。*/
    if (ins[maxID] && notadj[maxID] >= K - 1) /*如果顶点maxID已经被插入解集S且与顶点maxID不相邻的顶点数量大于等于K - 1*/
    {  /*如果与顶点maxID不相邻的顶点数量大于等于K，说明它不符合kplex的基本要求，则直接返回，不再继续搜索。*/
        if (notadj[maxID] >= K) /*这个if语句不同于前面的第一种情况，因为这里是表示顶点maxID已经被插入到了已选集中的。*/
            return;
        /*如果在这里没有返回，那么就说明这个maxID是一个饱和顶点！！！它已经濒临被剔除的边缘了！！这个时候只要maxID想要加入解集S，那么所有和它不相邻的候选集中的顶点都得被删除。*/
        ...
        {
            for (auto x : todel) /*那么就遍历待删除的顶点列表，并从候选集中全部删除这些顶点*/
                delfrD(x); /*因为在delfrD函数中设置了del标志为true，所以就相当于暂时的把这些顶点从图中删除了。*/
            dfs(curS - todel.size()); /*递归地调用dfs函数，这里就相当于是为maxID的搜索进行铺路，因为只要maxID在解集里面（没有被删）那么todel里面就不能有顶点。*/
            for (auto x : todel) /*如果前面的dfs没有能找到更大的s-bundle，那么我们再把刚刚从候选集中删除掉的顶点再依次加回候选集中去。*/
                addtoD(x); /*恢复先前删除的顶点到当前解中*/
            return;
        }
    }
    /*如果上面的情况都不符合（因为但凡符合了一种的话就已经提前return了），那么就看当前的解集是不是空的。*/
    if (svex.size())  /*如果 svex 非空，选择其中的一个随机顶点进行广度优先搜索（BFS），然后基于 BFS 的结果收集一组特定条件下的顶点进行进一步处理。*/
    {  
        ...
        /*下面的循环遍历图中的所有顶点 v，收集那些未被删除且满足特定条件的顶点。条件是顶点 v 要么在 BFS 中未被访问过（dis[v] == -1），要么其距离超过了某个阈值（dis[v] > max(2, K + K - LB)）
        。这个阈值用于图的规约。 如果顶点 v 已经被插入 (ins[v] 为真)，则函数提前返回。*/
        for (int v = 0; v < n; ++v) /*遍历当前所有的顶点*/
            if (!del[v]) /*如果顶点v未被删除*/
            { /*如果顶点v在广度优先搜索中未被访问过（dis[v]的值为-1，说明这个顶点和我们的顶点u不连通），或者说若是顶点v的距离超过了阈值max(2, K + K - LB)*/
                if (dis[v] == -1 || dis[v] > max(2, K + K - LB))  /*如果满足了上面的任何一个条件，则说明该顶点的距离已经超过了一个合格s-bundle所允许的最大距离了。*/
                {
                    if (ins[v]) /*如果这个顶点已经被插入解集S中了，那么就说明整个解集都是不符合s-bundle要求的（因为此时包含顶点v的图是一个不连通图）。此时直接返回。*/
                        return;
                    sofar.push_back(v); /*但如果顶点v未被插入，则将该不符合距离要求的顶点v添加到sofar数组中进行存储，便于后面统一删除。*/
                }
            }
        if (sofar.size()) /*如果sofar数组不为空，那么就把这些不合格的顶点都删掉， 然后再进行深度优先递归搜索。*/
        {
            for (auto x : sofar)
                delfrD(x); 
            dfs(curS - sofar.size()); /*递归地调用dfs函数，这次的递归就相当于专门为随机选择的顶点u进行的，因为只要它在，sofar里面就不能存在任何的顶点，否则就不是一个合格的解。*/
            for (auto x : sofar)  /*如果没有找到最优解，那么将删除的顶点重新加回到候选集中。*/
                addtoD(x); 
            return;
        }
    }
    /*如果上面的情况都不符合（因为但凡符合了一种的话就已经提前return了），那么就看当前基于图着色的上界。*/
    if (get_color() <= LB) /*如果根据染色法计算出来的上界upper bound比不上当前的下界，那么说明当前的分支可以被砍掉。*/
        return;
    /*如果上面的情况都不符合（因为但凡符合了一种的话就已经提前return了），那么就开始进行多分支操作。*/
    vector<int> branch; /*branch就是对应着最小度数顶点不相邻的顶点集合。*/  
    for (int x = 0; x < n; ++x) /*接下来开始进入多分支策略，每次都从当前图中度数最小的顶点出发，然后找和这个顶点不相邻的顶点集合。*/
        if (!del[x] && x != minID && !ins[x]) /*如果顶点x未被删除、且顶点x不是度数最小的顶点，同时顶点x未被插入解集中。*/
        {
            if (!adjN[minID].test(x)) /*如果该顶点x不是最小度数的顶点的邻居，也就是说和我们前面计算出来的最小度数的顶点不相邻。*/
    ...
    if (ins[minID]) /*当顶点minID已被插入时，就需要找它所能够加入的最大非邻居顶点数量。对应着原文中的分支规则2！*/
    {
        ...
        for (int i = 0; i < canselect; ++i) /*对branch 中的顶点执行操作，选择canselect数量个顶点*/
        {
            ...
            if (i && !canadd(branch[i - 1])) /*如果这个顶点不是第一个顶点，即i!=0，并且通过canadd判断分支顶点集合中的第i-1个顶点能否和当前第i个顶点一起被加入到解集中。这里是判断不能被同时加入。*/
            {
                addtoD(branch[i]); /*如果不能添加，将当前顶点重新添加回候选集中*/
                all = false;  /*将all设置为false，并跳出循环。all表示第i个顶点不能和前面已经分支过的顶点共存。*/
                break; /*提前终止多分支，这就是运用了多分支方法的一个好处，可以很方便的剪去后面的支。*/
            }
            if (i) /*如果没有被break掉的话，就把前面的i-1个顶点都加入到解集S中。*/
            {
        ...
        /*因为在上面的循环中，并没有对canselect里面的最后一个顶点进行判断，所以下面单独把这个特殊位置的顶点拉出来处理。其实这里就是对应着多分支规则的最后一个分支！！！即把最小度数顶点所有允许的可加入非邻居都剔除掉的那个分支！*/
        if (all && (canselect == 0 || canadd(branch[canselect - 1]))) /*如果canselect集合中所有顶点都没有产生冲突，同时要么canselect为0（意味着没有不相邻的顶点可以被用来分支），要么能够添加branch中最后一个顶点*/
        {
            ...
            if (canselect)                                       /*如果还有剩余的可以被选择的分支顶点*/
                addtoS(branch[canselect - 1]);                   /*将branch中的第canselect个顶点（最后一个可以被加入的顶点）加入到解集S中*/
            ...
            for (int i = canselect; i < (int)branch.size(); ++i) /*对于前面删除过的branch中除了canselect后面的剩余顶点，在结束递归后又重新加回去*/
                addtoD(branch[i]); /*重新将添加过的顶点放回到候选集中。看到这里了我可以发现，整个的操作都是非常对称的，先删除后加回。*/
        }
    ...
    { 
        ...
        addtoS(minID);      /*否则如果minID能够被添加到解集S中，则将其插入。注意！！！到了这个时候，又回到了第一个if分支里面的情况了！！！所以下面的代码和前面的大部分代码都是类似的。*/
        ...
        for (int i = 0; i < canselect; ++i)    /*依次遍历在已经把度数最小的顶点加入到解集S中后，最多还能添加的顶点数量*/
        {
            ...
            { 
                addtoD(branch[i]);  /*如果不能添加，则将当前顶点重新添加到候选集中，将all设置为false，并跳出循环*/
                all = false;
                break;
            }
            if (i) /*！！！注意，从这里我看出了！！原来这就是利用了多分支的剪枝！一旦前面的任何一个分支不符合要求，都直接返回了！！！*/
            { 
                addtoS(branch[i - 1]);  /*将上一个顶点插入到已选集中，并更新pos*/
                pos = i - 1;
            }
...
void work() /*核心函数，调用分支限界方法来求解max s-bundle问题。*/
{
    ...
    for (int i = 0; i < 16; ++i)/*初始化一个名为twoPow的数组。该数组的索引表示2的幂，而数组中的元素表示对应的指数。例如，twoPow[1]将被设置为0，twoPow[2]将被设置为1，twoPow[4]将被设置为2，以此类推。*/
        twoPow[1 << i] = i; /*这个映射关系通常用于处理二进制位操作。这个循环将twoPow数组初始化为一个指数映射表。这个数组的主要目的是让我们快速的找出某个数是2的几次方。这个操作在后面的位操作运算的时候特别有用。*/
    ...
    dfs(n); /*排完序之后执行深度优先搜索函数，这也是整个代码里面最核心的地方。注意，这时候传进去dfs里面的顶点数量n已经是经过缩减以后的数量了。*/
    exit_program(); /*用于退出程序*/
}
