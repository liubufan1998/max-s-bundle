...
inline void delfrS(int u)
{
    ...
    svex.pop_back(); /*这行代码从 svex 数组（可能是一个存储特定顶点集的向量）中移除最后一个元素。这似乎是基于假设，即 u 是 svex 中最近添加的顶点。*/
    for (int i = 0; i < n; ++i)
    {
        /*更新与顶点 u 不相邻的顶点数量*/
...
void dfs(int curS)
{
    ...
    {
        /*如果顶点 maxID 未被插入（由 ins 数组标记）且与 maxID 不相邻的顶点数量大于等于 K，则执行一系列操作。
        首先，从当前图结构中删除 maxID，然后对更新后的图递归地调用 dfs 函数，最后将 maxID 添加回图中。*/
        delfrD(maxID);
    ...
    /*理特定条件下的 maxID
    这部分代码处理 maxID 已被插入但与之不相邻的顶点数量满足特定条件的情况。
    如果条件满足，则收集一组待删除的顶点，并对这些顶点执行删除操作后递归调用 dfs 函数。*/
    ...
    if (svex.size())
    {
        ...
        {
            /*
            如果收集到的顶点集 sofar 非空，则对这些顶点执行一系列操作。
            首先，通过 delfrD 函数删除这些顶点。
            然后，递归地调用 dfs 函数，更新当前搜索深度 curS。
            最后，通过 addtoD 函数将这些顶点重新添加到数据结构中。
            */
            for (auto x : sofar)
                delfrD(x);
            dfs(curS - sofar.size());
    ...
    /*颜色获取与分支处理*/
    if (get_color() <= LB)
        /*这里首先检查通过 get_color 函数获取的值是否满足某个条件。如果不满足，函数返回。否则，收集一组用于分支处理的顶点。*/
        return;
    /*收集分支处理的顶点*/
    vector<int> branch;
    for (int x = 0; x < n; ++x)
    ...
    if (ins[minID])
    { // 当 minID 已被插入时
        /*这一部分代码处理 minID 已被插入的情况。
        计算 canselect，表示可以选择的顶点数量，基于 notadj[minID]（可能表示与 minID 不相邻的顶点数量）。
        对 branch 中的顶点执行一系列操作，包括删除 (delfrD)、添加 (addtoD) 和更改状态 (addtoS)，并递归调用 dfs。*/
        int canselect = K - 1 - notadj[minID], pos = -1; // pos 初始化为 -1，可能用于追踪处理过程中的某个特定位置。
    ...
    { // 当 minID 未被插入时
        ...
        addtoD(minID);
        if (!canadd(minID))
            return;    /*检查 minID 是否能被再次添加.如果 minID 不能被添加（由 canadd 函数检查），
则函数提前返回。这可能是一种优化，避免无效或重复的搜索。*/
        addtoS(minID); /*将 minID 插入到当前的解集S中*/
        /*处理 branch 中的其他顶点*/
...
void work()
{
    ...
    for (int i = 0; i < 16; ++i)
        twoPow[1 << i] = i;
    /*具体来说，这段代码的作用是初始化一个数组 twoPow，该数组的索引表示 2 的幂，而数组中的元素表示对应的指数。换句话说，它建立了一个映射关系，
    用于将 2 的幂映射到对应的指数值。例如，twoPow[1] 将被设置为 0，twoPow[2] 将被设置为 1，twoPow[4] 将被设置为 2，以此类推。
    这种映射通常用于一些算法中，特别是在处理二进制位操作时。例如，如果你想知道某个数是 2 的多少次方，你可以使用这个数组来查找。
    例如，如果 twoPow[16] 的值为 4，那么你可以知道 2^4 = 16。这对于一些位操作和计算中非常有用。 */

    /*调用PreProcess函数，并传递LB + 1作为参数。如果PreProcess返回false（可能表示预处理失败），则调用exit_program函数退出程序。*/
    if (!PreProcess(LB + 1)) // 如果 PreProcess 函数返回值为假（即返回值为 0），则执行以下操作：
        exit_program();
    getOrd(); /*获取顶点顺序或排序。*/
