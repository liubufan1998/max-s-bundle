...
/*这个函数的作用是实现了一个简单的顶点标识到内部标识的映射。如果顶点标识尚未映射，那么它将会被映射到一个新的、唯一的内部标识。
这个内部标识始终是唯一的，并且随着映射的进行而增加。*/
map<int, int> mp;
inline int find_id(int u)
{
    auto it = mp.find(u); /*函数通过调用mp.find(u)来查找键为u的元素是否存在于mp中。find函数返回一个指向该元素的迭代器。
    如果u已经在mp中，那么这个迭代器会指向该元素，否则它将会指向mp.end()。*/
    if (it != mp.end()) /*如果键u已经在mp中*/
        return it->second; /*，那么函数就直接返回这个键对应的值，这就是映射的结果。*/
    /*如果键u不在mp中，那么函数首先获取mp的大小（即当前已经映射的键的数量），然后使用这个大小作为新的键u的值添加到mp中。
    这样，即使之前没有映射过u，现在也已经映射了，并且新的映射键值对中的值是唯一的，它等于当前映射的键的数量。*/
    int sz = mp.size();
...
struct BIT_SET
{
    ...
    void init(int _n) /*初始化一个位图数据结构，根据输入的位数 _n，确定位图需要多少个 unsigned 整数来存储，然后分配相应的内存空间，并将位图的数据初始化为全 0.这样子的分配效率的确高多了！*/
    {
        ...
        m = _n & 31;
        /*这行代码计算 _n 的低 5 位，它使用了位运算 &（按位与），31 的二进制表示为 11111，所以 & 操作可以提取 _n 的低 5 位，结果赋值给变量 m。这个操作的目的是确定每个 unsigned 整数中的位数，通常是 32 位。*/
        n = _n >> 5;
    ...
    void flip() /*对位图中的每个 unsigned 整数执行按位取反操作，*/
    {
       ...
       例如，如果 m 等于 5，那么 (1 << m) - 1 的二进制表示为 0000011111。^：这是异或运算符. */
    ...
    bool test(int x) /*检查位集合中特定位置 x 的位是否设置。*/
    {
...
/*这里声明了 adjN 和 invN 作为 BIT_SET 类型的指针。在 C++ 中，指针是一个变量，它存储了另一个变量的内存地址。这里，adjN 和 invN 是指向 BIT_SET 类型对象的指针。
...
public:
    ...
    bool solve(bool cals, int bound) /*传入的两个参数中，第一个参数是是否计算当前解集svex的连通性；第二个参数是当前的下界bound*/
    {
        ...
        if (cals)   /*如果 cals 为真，则遍历 svex 向量中的每个顶点 u*/
        {
            ...
            for (int i = 0; i < nn; ++i) /*对于 nn 范围内的每个顶点 i，如果它不是顶点 u，并且 u 不与 i 邻接， 则运行 SAP 算法计算从超级源点 u + nn 到 i 的最大流。*/
            {
...
bool PreProcess(int S)
{
    ...
    while (fr ^ re) /*这段代码通过队列实现了类似宽度优先搜索的操作，用于进一步处理顶点和边的关系。这个符号是异或操作，异或操作的技巧是不同为1，相同为0。*/
    {                      
    ...
    if (n < S) /*如果处理后的顶点数量 n 小于阈值 S，函数返回 false。这里的S是我们传入的下界，也就是说至少要这么多顶点数目才有意义。*/
        return false;

    /*下面代码遍历所有边，并对边进行过滤。如果边的任一端点被标记在 outcore 中，则该边被忽略。
        对于未被忽略的边，更新它们的端点为新的编号，并将其存储回 E 数组。这样，E 数组中的边仅包含未被 outcore 标记的顶点。
        m 计数未被忽略的边的数量，并最终用于更新 M，即图中边的总数。所以说outcore的意思就是不符合要求的顶点。*/
    int m = 0; /*初始化边的计数器 m。*/
    for (int i = 0; i < M; ++i)
    {/*循环遍历所有的边（由数组 E 存储）*/
        int u = E[i].first, v = E[i].second;
...
void exit_program() /*退出程序的时候打印信息：（1）文件的名称filename；（2）s-bundle的s的值K；（3）深度优先搜索经历过的顶点数量dfs_node；
（4）我们传入的s-bundle的下界LB；（5）算法运行所消耗的时间。*/
{
    printf("Filename: %s\n", filename);
    printf("Value of K: %d\n", K);
...
inline void delfrS(int u) /*delfrS表示把一个原本在解集S中的顶点拿出来又放回到集合D中去，通常用于回溯操作。*/
{
    ...
    for (int i = 0; i < n; ++i) /*更新与顶点 u 不相邻的顶点数量*/
    {
...
void get_color(vector<pair<int, int>> &lst) 
{
    ...
    { /*P.empty() 检查 P 是否没有任何设置的位。*/
        ...
        do
        {
            ...
            Q.set(u);
            Q &= invN[u]; /*更新 Q 为 Q 与 invN[u] 的按位与，其中invN是非邻接顶点位集合，用来快速判断两个顶点是否是不相邻。invN[u]表示和顶点u不相邻的顶点集合。
...
void dfs_kbundle(int curS, bool check) /*第一个参数表示当前的顶点集合大小；第二个参数表示是否要检查当前子图svex的连通性。*/
{
    ...
    if (curS <= LB)
        return;
    if (check && vc.solve(false, curS - K)) /*如果 check 为真且 vc.solve(false, curS - K) 返回真（意味着当前的curS构成的图是连通的，则将下界LB 设置为 curS 并返回。*/
    {
        LB = curS; /*现在的LB就是真的和s-bundle相关的下界了*/
        return;
    }
    /*获取颜色列表并进行检查*/
    vector<pair<int, int>> lst; /*定义了一个顶点对集合，这个集合里面应该是存储了最少的染色数量集合。*/
    ...
    /*处理列表中的元素*/
    for (int i = m - 1; i >= 0; --i) /*进行反向遍历。注意了！这里是进行的反向遍历！也就是先从最后一个元素遍历，这时候的lst[i]是最大的。*/
    {
        if ((int)svex.size() + lst[i].second <= LB) /*lst.second存放的是和顶点数量相关的。这个if语句表达的意思是如果lst[i]无法取得一个比当前下界更优的解的时候，就把
        剩余的顶点都加入到集合D中。*/
        {
            for (int j = i + 1; j < m; ++j)
                addtoD(lst[j].first); /*addtoD表示把一个顶点添加到删除顶点集合中去，D是delete的首字母缩写。*/
            return;
        }
...
void dfs(int curS) /*传入的参数curS是当要搜索的顶点集合的大小。*/
{
    ...
    if (ins[maxID] && notadj[maxID] >= K - 1) /*检查顶点maxID是否已经被插入（已被标记为已插入）且与顶点maxID不相邻的顶点数量是否大于等于K - 1*/
    {
        ...
        if (todel.size()) /*如果待删除的顶点列表中有顶点*/
        {
            for (auto x : todel) /*遍历待删除的顶点列表，并依次从候选集中删除这些顶点*/
                delfrD(x);
            dfs(curS - todel.size()); /*递归地调用dfs函数，减少当前待搜索空间的大小*/
    ...
    if (svex.size()) /*如果当前的解集svex不为空*/
    {
        ...
        for (int v = 0; v < n; ++v) /*遍历当前所有的顶点*/
            if (!del[v]) /*如果顶点v未被删除*/
            {
                if (dis[v] == -1 || dis[v] > max(2, K + K - LB)) /*顶点v在广度优先搜索中未被访问过（dis[v]的值为-1），且顶点v的距离超过了某个阈值，这个阈值是max(2, K + K - LB)*/
                {/*如果满足了上面的任何一个条件，则说明该顶点的距离已经超过了一个合格s-bundle所允许的最大距离了。*/
                    if (ins[v]) /*如果这个顶点已经被插入解集S中了，那么就说明整个解集都是不符合s-bundle要求的。此时直接返回。*/
                        return;
    ...
    if (get_color() <= LB) /*如果根据染色法计算出来的上界upper bound比不上当前的下界，那么说明当前的分支可以被砍掉。*/
        return;
    /*收集分支处理的顶点*/
    vector<int> branch; /*在这里创建一个整数向量branch，用于存储待处理的顶点。*/
    ...
    for (int x = 0; x < n; ++x)
        if (!del[x] && x != minID && !ins[x]) /*如果顶点x未被删除、且顶点x不是度数最小的顶点，同时顶点x未被插入*/
        {
            ...
            if (!adjN[minID].test(x)) /*如果该顶点x不是最小度数的顶点的邻居。这里使用adjN[minID].test(x)来检查顶点之间是否有边相连。*/
                branch.push_back(x);  /*那么就把该顶点x添加到branch数组中*/
        }

    /*处理 minID 对应的顶点，这里就是对应的原文中的多分支策略，每次都是选择的度数最小的这个顶点。*/
    if (ins[minID]) /*当顶点minID已被插入时*/
    {
        int canselect = K - 1 - notadj[minID], pos = -1;/*首先计算canselect，它表示可以选择的顶点数量, 初始化pos为-1。*/
        bool all = true; /*all 初始化为 true，可能用于标记是否所有顶点都符合某个条件。*/
        /*对 branch 中的顶点执行操作*/
        for (int i = 0; i < canselect; ++i) /*选择canselect数量个顶点*/
        {
            ...
            delfrD(branch[i]); /*正是对应着多分支的策略，即把第i个顶点从候选集中删掉。*/
            if (i && !canadd(branch[i - 1])) /*通过canadd判断分支顶点集合中的第i-1个顶点能否和当前第i个顶点一起被加入到解集中。这里是判断不能被同时加入。*/
            {
            ...
            dfs(curS - 1); /*递归调用dfs函数，更新搜索深度为当前的顶点数量减去1*/
    ...
    else /*当度数最小的顶点没有被存放到解集S中时应采取的动作。*/
    { 
        ...
        addtoS(minID);      /*如果minID能够被添加到解集S中，则将其插入*/
        /*处理 branch 中的其他顶点*/
...
void work()
{
    ...
    LB = max(wG, K); /*这样设置的原因是s-bundle的最起码大小要为s，也就是这里的K。我感觉这个作者写的代码太烂了，远远不如常老师、师弟和开强的代码质量，后面我要好好的重构。*/

    /*初始化一个名为twoPow的数组。该数组的索引表示2的幂，而数组中的元素表示对应的指数。例如，twoPow[1]将被设置为0，twoPow[2]将被设置为1，twoPow[4]将被设置为2，以此类推。
    这个映射关系通常用于处理二进制位操作。这个循环将twoPow数组初始化为一个指数映射表。这个数组的主要目的是让我们快速的找出某个数是2的几次方。*/
    for (int i = 0; i < 16; ++i)
        twoPow[1 << i] = i;

    /*preProcess的主要作用是根据顶点的度数是否符合kplex来进行删点的操作。*/
