...
public:
    ...
    例如前向边和反向边、层次图（level graph）、以及GAP优化。该算法会返回一个int类型res，表示最大流的结果，也就是最小割的数值。*/
    int SAP(int start, int end) /*注意了！！在求解具有连通度属性的max s-bundle问题的时候，最重要的一个定理依据就是“最大流最小割”定理！也就是说s-bundle是想找出来最小割的大小，
    然而我们要是直接求解最小割的话会非常的不方便，所以说我们就求解了最大流，因为最大流问题有非常成熟的方法来进行求解。该算法采用的是Ford-Fulkerson算法和SAP（Shortest 
    Augmenting Path，最短增广路径）算法的结合*/
    {
        /* 恢复每条边的初始容量，为新的流计算做准备*/
        for (int i = 0; i < tol; ++i)
            edge[i].cap = cap_backup[i];
        int res = 0; /*初始化结果变量res为0，这个变量用来存储最大流的流量*/
        BFS(start, end); /*使用BFS（广度优先搜索）构建一个层次图*/
        int top = 0; /*记录当前遍历到的边*/
        memcpy(cur, head, n * sizeof(int)); /*复制head数组到cur数组。*/
        int u = start; /*初始化u为源节点*/
        int i;
        while (dep[start] < n) /*当源节点所在的层次小于n时，继续循环*/
        {                      
            ...
            else /*如果当前节点没有合适的边，则退回到前一个节点，并更新层次*/
            {
                ...
                for (i = head[u]; i != -1; i = edge[i].next) /*遍历当前节点u的所有前继节点。*/
                {
                    if (edge[i].cap == 0) /*如果当前边的容量为0，则跳过后续代码，继续遍历下一个前继节点。*/
                        continue;
...
/*这个函数通过 SAP 算法来判断移除一定数量的顶点后图是否仍然连通。如果移除少于 bound 个顶点后图不再连通，则认为图的顶点连通性小于 bound。*/
...
void dfs(int curS) /*传入的参数curS是当要搜索的顶点集合的大小。*/
{
    ...
    if (ins[minID]) /*当顶点minID已被插入时，就需要找它所能够加入的最大非邻居顶点数量。*/
    {
        ...
        for (int i = 0; i < canselect; ++i) /*对branch 中的顶点执行操作，选择canselect数量个顶点*/
        {
            ...
            delfrD(branch[i]); /*正是对应着论文中多分支的策略，即把第i个顶点从候选集中删掉。*/
